...
class GrantTable(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('pending', 'active', 'expired', 'revoked')", name='valid_status'),
        CheckConstraint("source IN ('rental_control', 'voucher', 'manual')", name='valid_source'),
...
class VoucherTable(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('active', 'inactive', 'expired')", name='valid_voucher_status'),
...
class ThemeConfigTable(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("LENGTH(background_color) = 7 AND background_color LIKE '#%'", name='valid_bg_color'),
        CheckConstraint("LENGTH(primary_color) = 7 AND primary_color LIKE '#%'", name='valid_primary_color'),
    )