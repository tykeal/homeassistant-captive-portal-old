...
class AccessGrant(BaseModel):
    ...
    booking_id: Optional[str] = Field(None, description="Rental Control booking ID")
    ...
    device_mac: Optional[str] = Field(None, description="Device MAC address")
    
    # Controller integration
    controller_voucher_id: Optional[str] = Field(None, description="Controller voucher ID")
    ...
    def is_currently_active(self) -> bool:
        ...
        return (
            self.status == GrantStatus.ACTIVE and
    ...
    def should_be_activated(self) -> bool:
        ...
        now = datetime.now(timezone.utc)
        return (
            self.status == GrantStatus.PENDING and
            now >= self.start_time
        )
    ...
    def should_be_expired(self) -> bool:
        ...
        now = datetime.now(timezone.utc)
        return (
            self.status in [GrantStatus.PENDING, GrantStatus.ACTIVE] and
            now > self.end_time
        )
    ...
    def shorten_grant(self, new_end_time: Optional[datetime], reason: str, immediate: bool = False) -> None:
...
class EventLogEntry(BaseModel):
    ...
    # Context information
    entity_type: Optional[str] = Field(None, description="Type of entity (grant, voucher, etc.)")
    ...
    # User context
    user_id: Optional[str] = Field(None, description="User or system identifier")
...
class ThemeConfig(BaseModel):
    ...
    updated_by: Optional[str] = Field(None, description="Who updated the theme")
    ...
    @classmethod
    def validate_custom_css(cls, v: Optional[str]) -> Optional[str]:
