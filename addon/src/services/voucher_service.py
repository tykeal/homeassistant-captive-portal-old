...
class VoucherService:
    ...
    async def create_voucher(
        ...
        **audit_context
        ...
        raise RuntimeError("Failed to generate unique voucher code after maximum attempts")
    
    async def get_voucher_by_id(self, voucher_id: str) -> Optional[Voucher]:
        """Get voucher by ID.
    ...
    async def validate_voucher_code(self, code: str) -> tuple[bool, Optional[str], Optional[Voucher]]:
    ...
    async def expire_vouchers(self) -> int:
        ...
        if expired_count > 0:
            logger.info(
                "Voucher expiry check completed",
        ...
        for voucher in all_vouchers:
            ...
            if voucher.status == VoucherStatus.ACTIVE and voucher.is_valid():
                if voucher.max_uses == -1:
                    stats["available_uses"] = float('inf')
