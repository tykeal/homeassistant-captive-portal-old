...
from .queue_scheduler import get_queue_scheduler, TaskPriority
from .grant_manager import get_grant_manager
from .voucher_service import get_voucher_service
from .audit_logger import get_audit_logger
from ..models.domain import AccessGrant, Voucher
...
class QueuedOperations:
        ...
        async def _activate_grant() -> Dict[str, Any]:
            ...
            try:
                ...
                return {
                    ...
                    "activated_at": activated_grant.activated_at.isoformat() if activated_grant.activated_at else None
                }
        ...
        async def _revoke_grant() -> Dict[str, Any]:
            ...
            try:
                ...
                return {
                    ...
                    "revoked_at": revoked_grant.revoked_at.isoformat() if revoked_grant.revoked_at else None,
                    "reason": reason
                }
    ...
    async def queue_voucher_cleanup(
        ...
        await self.scheduler.submit_task(
            ...
            priority=priority,
            context={
                "operation": "voucher_cleanup"
            }
        )
        
        logger.debug(
            "Voucher cleanup queued",
            task_id=task_id,
            priority=priority.name
        )
    ...
    async def queue_grant_lifecycle_processing(
        ...
        await self.scheduler.submit_task(
            ...
            priority=priority,
            context={
                "operation": "grant_lifecycle_processing"
            }
        )
        
        logger.debug(
            "Grant lifecycle processing queued",
            task_id=task_id,
            priority=priority.name
        )
...
_queued_operations: Optional[QueuedOperations] = None
