...
from ..models.domain import AccessGrant, GrantStatus, GrantSource
...
class GrantManager:
    ...
    async def get_grant_by_id(self, grant_id: str) -> Optional[AccessGrant]:
        """Get grant by ID.
    ...
    async def get_grant_by_booking_id(self, booking_id: str) -> Optional[AccessGrant]:
    ...
    async def expire_grant(
        ...
        **audit_context
        ...
        # Log expiration
        await self.audit_logger.log_grant_expired(
            grant=grant,
            **audit_context
        )
    ...
    async def update_grant_error(
        ...
        if increment_retry:
            grant.retry_count += 1
        grant.modified_at = datetime.now(timezone.utc)
    ...
    async def process_grant_lifecycle(self) -> Dict[str, int]:
        ...
        try:
            ...
            if stats["activated"] > 0 or stats["expired"] > 0:
                logger.info(
                    "Grant lifecycle processing completed",
                    **stats
                )
        
        except Exception as e:
            logger.error(
                "Grant lifecycle processing failed",
                error=str(e)
            )
            stats["errors"] += 1
